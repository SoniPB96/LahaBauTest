...
export default function HomePage() {
  return (
    <>
      ...
      <div className="max-w-site mx-auto px-6 md:px-7 pt-20 md:pt-24 pb-16 md:pb-20">
        <div className="grid md:grid-cols-2 gap-12 md:gap-14 items-center">
          ...
          <div className="hidden md:block">
            <div
              className="bg-bg-3 border rounded-xl p-7 relative overflow-hidden"
            ...
            >
              <div
                ...
                style={{
                  background:
                    'linear-gradient(90deg,transparent,rgba(201,170,114,0.35),transparent)',
                }}
              />
              ...
              <div className="flex flex-col gap-2 mb-5">
                {['Sanierung / Renovierung', 'Modernisierung', 'Neubau / Erweiterung'].map(
                  (s, i) => (
                    <div
                      key={s}
                      className={`flex items-center gap-3 px-4 py-3 rounded-lg border transition-colors ${
                        i === 0
                          ? 'bg-bg-4 border-muted'
                          : 'border-transparent hover:border-subtle'
                      }`}
                      style={i !== 0 ? { borderColor: 'transparent' } : undefined}
                    >
                      <span
                        className={`w-[18px] h-[18px] rounded-full border flex items-center
                                    justify-content shrink-0 ${
                                      i === 0
                                        ? 'bg-gold border-gold'
                                        : 'border-muted'
                                    }`}
                        style={{ borderColor: i === 0 ? '#c9aa72' : 'rgba(255,255,255,0.11)' }}
                      />
              ...
              <Link
                ...
                className="flex items-center justify-between px-4 py-3 rounded-lg border
                           bg-gold-tint border-gold hover:bg-gold/10 transition-colors no-underline"
                style={{
                  backgroundColor: 'rgba(201,170,114,0.08)',
                  borderColor: 'rgba(201,170,114,0.22)',
                }}
              >
                <span className="text-[0.82rem] text-gold font-medium">
      ...
      {/* ── TRUST BAR ── */}
      <div
        className="border-y bg-bg-2 py-3.5"
        style={{ borderColor: 'rgba(255,255,255,0.06)' }}
      >
        <div className="max-w-site mx-auto px-6 md:px-7 flex gap-6 flex-wrap justify-center">
          {TRUST_ITEMS.map((item) => (
            <span key={item} className="flex items-center gap-2 text-[0.72rem] text-text-3
      ...
      <SectionContainer>
        ...
        <div className="grid sm:grid-cols-2 gap-5">
          {SERVICES.map(({ title, desc, tags }) => (
            <div
              key={title}
              className="bg-bg-2 border rounded-lg p-7 transition-colors
                         hover:border-muted hover:bg-bg-3"
              style={{ borderColor: 'rgba(255,255,255,0.06)' }}
            >
              ...
              <div className="flex flex-wrap gap-1.5">
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-[0.65rem] text-text-4 border rounded px-2 py-0.5
                               tracking-[0.04em]"
                    style={{ borderColor: 'rgba(255,255,255,0.06)' }}
                  >
                    {tag}
      ...
      >
        ...
        >
          {[
            ...
            <div key={label} className="py-8 sm:py-0 sm:px-10 text-center first:pt-0 last:pb-0">
      ...
      <SectionContainer size="sm">
        ...
        />
        <div
          className="border rounded-xl p-8 md:p-10 grid md:grid-cols-2 gap-10 items-center bg-bg-2"
          style={{ borderColor: 'rgba(255,255,255,0.06)' }}
        >
              ...
              <div key={label} className="flex items-center gap-3">
                <div
                  className={`w-5 h-5 rounded-full border flex items-center justify-center
                               text-[0.62rem] shrink-0 ${
                                 i < 2
                                   ? 'bg-gold border-gold text-[#1a1400] font-medium'
                                   : 'border-muted text-text-4'
                               }`}
                  style={{ borderColor: i < 2 ? '#c9aa72' : 'rgba(255,255,255,0.11)' }}
                >
                  {i + 1}
                </div>
                <span
                  className={`text-[0.82rem] ${i < 2 ? 'text-text-2' : 'text-text-3'}`}
                >
                  {label}
                </span>
              </div>
